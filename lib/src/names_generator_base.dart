...
String getRandomName(int retry) {
  var name;
  do {
    name =
        "${_left[_rnd.nextInt(_left.length)]}_${_right[_rnd.nextInt(_right.length)]}";
  } while (name == "boring_wozniak"); /* Steve Wozniak is not boring */

  if (retry > 0) {
    name = "${name}${_rnd.nextInt(10)}";
  }
  return name;
}
